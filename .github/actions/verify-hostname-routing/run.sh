...
for ((ATTEMPT = 1; ATTEMPT <= MAX_ATTEMPTS; ATTEMPT++)); do
  ...
  if curl "${CURL_ARGS[@]}" "https://$PRIMARY_DOMAIN/"; then
    ...
    echo "$PRIMARY_DOMAIN responded, but not identifiably from Kinsta or Cloudflare (attempt $ATTEMPT/$MAX_ATTEMPTS)."
...
echo "$PRIMARY_DOMAIN is not served by Kinsta after $MAX_ATTEMPTS attempts." >&2
echo "The DNS record is missing, still propagating, or points elsewhere." >&2
exit 3
